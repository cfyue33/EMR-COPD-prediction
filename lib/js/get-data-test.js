...
//calculate the age and gender score for the prediction
...
function smokingCal () {
 ...
 //document.getElementById('predict').addEventListener("click", copdPredict(genderScore, ageScore, smokingScore));
//document.getElementById('predict').addEventListener("click", displayPredictioncopdPredict(genderScore, ageScore, smokingScore));

// prediction algorithm
function copdPredict (gender, age, smoking) {
	var prob = -0.719518 + 0.099568*gender + 0.017798*age + 0.065001*smoking;
	/*if (prob > 0.6) {
		result.innerHTML = "The Patient is predicted having COPD";
	} else if (prob <= 0.6) {
		result.innerHTML = "The Patient is predicted not having COPD";
	} */
    console.log(prob);
    return prob;	
}
...
client.request(`Patient/${client.patient.id}`).then(
  function(patient) {
    ...
    console.log(patient);
	//displayPrediction(patient);
	//document.getElementById('predict').addEventListener("click", copdPredict(genderCal(patient), ageCal(patient), smokingCal()));
	document.getElementById('predict').addEventListener("click", function () {
		var prob = copdPredict(genderCal(patient), ageCal(patient), smokingCal());
...
query.set("_sort", "-date");  //sort by decreasing date
query.set("code", [
...
// only display the first entry of coding array, this works if it only contains one element.
//			displayMedication(resource['medicationCodeableConcept']['coding'][0]['display']);
// display all element in each coding.
