...
//client.request("/MedicationRequest?patient=" + client.patient.id).then(
//medResults.forEach(function(med) {
	//var medStatus = med.find.MedicationRequest.status; 
	//if (med.status == "active" || med.status == "stopped") {
	  //var medName = med.medication;
//	  displayMedication(med);
//	  console.log(med);  // Fn + f12 in chrome to look at console
//  }
//})
//);
...
//  var annotationText = findViewById("annotation");  
//  annotation = annotationText.getText().toString();
  var annotation = document.getElementById('annotation').value;
  var authorString = 'Crystal';
  var date = new Date();
...
////////////////////////////////////////////////////////////////////
var weightQuery = new URLSearchParams();

weightQuery.set("patient", client.patient.id);
weightQuery.set("_count", 100);
weightQuery.set("_sort", "-date");  //sort by decreasing date
weightQuery.set("code", [
  'http://loinc.org|29463-7',  //weight
].join(","));
...
client.request("Observation?" + weightQuery, {
  ...
  function(ob) {
	...
	alert(getQuantityValueAndUnit(weight[0])); 
	alert(weight[0].id); 
